...
그럼 어떻게 일관된 아름다움을 아키텍처를 가져올 수 있을까. 여러 작업자들 간 얼마나 같은 생각을 하고 있는지가 중요하다. 나아가 단발적인 변화가 또다른 파편화를 만들지 않도록 전체 아키텍쳐 관점에서 신중하게 변화를 줘야 한다. 혹은 변화들을 특정 영역에 한정지어 전체 아키텍쳐 맥락을 잃어버리지 않도록 노력해야 한다. 무수한 파편화 속에서 예측가능한 설계의 인지가 불가능한 상태로 접어든 프로젝트는 유지보수가 매우 힘들다.
